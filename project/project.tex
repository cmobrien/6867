...
\begin{tabular}{c}
\begin{subfigure}[b]{3in}
	\includegraphics[width=3in]{plots/dist3.png}
	\caption{CAPTION}
\end{subfigure}
\\
\begin{subfigure}[b]{3in}
	\includegraphics[width=3in]{plots/dist6.png}
	\caption{CAPTION}
\end{subfigure}
\\
\begin{subfigure}[b]{3in}
	\includegraphics[width=3in]{plots/dist9.png}
	\caption{CAPTION}
\end{subfigure}
\end{tabular}
\caption{TEST}
...
It is also interesting to look at how the distributions over grades vary between the students. We attempt to represent this information graphically in Figure xx. Recall that for each student, we found the maximum likelihood estimate, and then classified their grade according to that. We are interested in a few things.

In each subfigure, we display the proportion of students who had an maximum likelihood of at least a certain value, for students classified correctly and incorrectly, for both the ordered logit and probit models. The different subfigures represent different points throughout the semester. 

First, we'd like to know how the values of the maximum likelihoods differed for those students that we were able to classify correctly, and those who we classified incorrectly. We can see that both the ordered logit and probit consistently tended to place higher probabilities on the students that they were ultimately able to classify correctly. This is comforting. It means that the models, in general, claimed to be less sure about the ones which they got wrong than the ones they got right.

We're also interested in how the maximum likelihoods differed between the ordered logit and probit models. This is the first time where we have seen a significant different in the performance of the two models. We can see that the ordered logit model tends to assign higher maximum likelihoods than the ordered probit model, particularly at the beginning and end of the semester.
...
DISCUSS VALUES OF WEIGHTS A LITTLE
